...
条件相近时，已安装项排在目录候选项之前。存在严重风险、与目标 Harness 不兼容，或与已安装内容重复时，候选项不会进入安装建议。算法 v7 与结果格式 v4 保留范围明确的英文排除说明和非名称命中的双词门槛，并对一组有限的简体/繁体中文概念做确定性归一化，覆盖长会话、需求演变和上下文压缩意图。能力缺口是高置信的 Skill 搜索提示：名称匹配只有在候选名称能贡献具体能力概念时才足以佐证提示，否则候选元数据必须达到具体概念的多词相关性门槛；泛化的单词名称不能只靠名称相同来绕过门槛。没有可信候选的简短任务只使用保守的非泛化词回退。任务展示别名、候选项的正向元数据和已选项的正向覆盖会进入同一个 gap-only canonical 命名空间；`do not use` 否定子句既不能佐证提示，也不能把提示标记为已覆盖。canonical 去重先于六项上限。未分词且只由泛化汉字组成的两字片段属于低置信输入，不会形成路由、名称匹配或孤立缺口。这些展示规则不会改变推荐评分。它仍是词法路由，不是通用的跨语言语义理解。结果会展示相关性、独特覆盖、风险、冗余、上下文估算、来源版本、兼容性和易读原因。

需要把结果交给 Harness 或配套 Skill 时，使用 `--compact-json`。它输出单行且不超过 4,096 UTF-8 字节，只保留选中的使用/安装建议与稳定警告码，不包含原始任务。`--json` 返回完整的 `PreflightResult`，包括候选决策、评分、特征、原因、冲突、盘点警告、能力缺口和汇总覆盖率；可安装的目录候选项仍可带有自身的 `source` 元数据，但不包含原生盘点的来源、所有权、插件或可见状态记录。资产报告与 Dashboard 会保留这些记录；Preflight 使用已经解析好的可见状态，并通过候选原因码和盘点警告表达相关结果。配套 Hook 仍以 2,048 字节为上限。
...
Skill Steward 接入 Codex 和 Claude Code 的 `UserPromptSubmit` 与结束 Hook，使用本地缓存，出错时不阻断 Harness。两者收到的是精简推荐，不包含原始任务文本或目录 URL；Codex 可能要求完成原生信任检查。GitHub Copilot CLI 明确为仅观察：Hook 只接收生命周期事件，推荐仍通过配套 Skill 或显式 CLI 预检获取。
...
三种适配器都使用临时 HOME 目录测试，并保留无关配置。“仅观察”是当前明确边界：本版本不会把推荐注入 Copilot 提示词。
...
- 安装、回滚和集成修改共用跨进程锁；集成应用还会在报告就绪前写入一份可供 Hook 使用的资产缓存。
...
- 任务评分仍是确定性的词法基线。算法 v7 只规范一组范围有限的简体/繁体中文概念与高置信能力缺口提示，不等于通用的跨语言语义理解，也不衡量真实任务成败。
