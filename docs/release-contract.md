...
The contract does not replace the existing protected environment, exact-`main` requirement, full native artifact set, byte-identity preflight, provenance, or one-time bootstrap controls described in [Native package publication](native-publication.md).
