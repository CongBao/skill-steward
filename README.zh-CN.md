...
这三步都是只读操作，不需要为了体验盘点和推荐而修改任何 Skill。以后真正要安装、改策略、隔离或接入 Hook 时，先停在预览结果，确认无误后再执行其中给出的完整命令。
...
条件相近时，已安装项排在目录候选项之前。存在严重风险、与目标 Harness 不兼容，或与已安装内容重复时，候选项不会进入安装建议。算法 v3 还能识别 `do not use this skill for PDFs` 这类范围明确的英文说明；如果 Skill 名称没有直接命中，至少需要两个任务词相符才会进入候选。结果会展示相关性、独特覆盖、风险、冗余、上下文估算、来源版本、兼容性和易读原因；中文任务的能力缺口也会按词组显示，不再暴露分词碎片。
...
CLI 和 Dashboard 发起的集成修改使用同一把跨进程锁。遇到忙碌状态时，本次调用不会改写 Harness 文件，也不会消耗已经审核的计划；等正在执行的修改结束后，可以继续使用原计划。无关设置与 Hook 会保留；如果托管配置被外部修改，移除操作会停止：
...
- CLI 修改计划保存在私有目录、会过期且只能使用一次；确认命令不会根据原始参数临时重建计划。
...
- 任务评分仍是确定性的词法基线。算法 v3 修复了本次实测中的意图错误，但不等于语义理解，也不衡量真实任务成败。
...
版本变化见 [CHANGELOG.md](CHANGELOG.md)；本次真实环境、发现、分数和优先级见 [2026-07-03 产品评审](docs/product-review-2026-07-03.md)。
