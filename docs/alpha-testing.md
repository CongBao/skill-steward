# Beta Candidate Testing Protocol
...
Target build: the active Beta release candidate in [`release-contract.json`](../release-contract.json). Confirm it with `pnpm release:check`. This remains a local Harness companion, not a Harness. Native semantics are tested only on the documented core adapter surfaces below.
...
| Local candidate bundle | `CI=true pnpm --filter skill-steward exec vitest run tests/local-candidate.test.mjs`; Windows CI also runs `pnpm candidate:install` into a disposable prefix | The current macOS/Linux runtime gets exactly one version-bound native helper plus the verified CLI; a stale Alpha helper is replaced and cannot satisfy Beta capability checks. Candidate output never recursively deletes an old directory, its stored manifest uses relative artifact names, and installation re-verifies the installed package and native metadata. Windows installs and smokes the CLI without claiming lifecycle-write support. |
| Protected CLI publication | `CI=true pnpm --filter skill-steward exec vitest run tests/verifier.test.mjs tests/cli-package-publisher.test.mjs tests/cli-publication-workflow.test.mjs tests/registry-install-smoke.test.mjs` | One source-bound verifier gates package tests, CI, and publication. `--check-only` is offline; all six native versions precede CLI registry access; byte-identical versions resume; conflicts refuse; only manual exact-main `cli-publish` can write. Reviewed publication is followed by clean Linux, macOS, and Windows registry installs. See [CLI npm publication](cli-publication.md). |
...
Before judging repository behavior through a global install, run `pnpm candidate:install` from a clean checkout even when `skill-steward --version` already matches `release-contract.json`; two candidate builds can share a version before publication. The command writes verified artifacts to a fresh private temporary directory unless `--output` names a new empty physical directory. The installer builds and verifies the CLI plus only the current macOS/Linux helper, or the non-mutating CLI on Windows, then reopens the installed manifests and native metadata before reporting success. Package and binary metadata bind the helper to the exact CLI version so a retained Alpha helper cannot satisfy Beta lifecycle capability checks. Start once with a clean `SKILL_STEWARD_HOME` and confirm the first-use path explains scan, Preflight, and dashboard without assuming existing state.
...
node scripts/verify-cli-package.mjs artifacts/npm/skill-steward-*.tgz
node scripts/verify-cli-package.mjs artifacts/pnpm/skill-steward-*.tgz
```
