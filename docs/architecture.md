...
flowchart LR
  ...
  Services --> Preflight[Preflight algorithm v9 / schema v5]
...
- `packages/preflight` combines installed and cached catalog candidates, then applies lexical relevance, versioned capability evidence, risk, redundancy, compatibility, context cost, and installation penalties. Algorithm v9 extracts at most 16 bounded actions, objects, and local action-object pairs from positive English/Chinese task and routing text. An exact pair can make a candidate plausible; a broad object, generic overlap, project scope, or partial capability match cannot. Greedy selection tracks uncovered task capabilities, rejects candidates that add neither a term nor a capability, and uses capability overlap before route-term Jaccard for redundancy. The earlier corroborated pre-merge trigger remains available as a narrow lexical signal. One internal polarity module removes bounded English and Chinese negative clauses before lexical matching, capability extraction, and gap projection. Harness ineligibility and critical risk remain vetoes. Capability details stay package-private; full results expose only numeric coverage/precision, a three-state trigger confidence, and stable reasons. The analyzer has no filesystem or network I/O and is not general semantic understanding.
...
The Codex and Claude Code adapters run `skill-steward hook prompt` when a user submits a prompt. The command reads the latest installed-portfolio report and cached catalog index, calls deterministic Preflight algorithm v9 with result schema v5, and emits at most 2,048 bytes of additional context. Algorithm v9 combines the existing ordered lexical/polarity path with the bounded capability grammar described above. A non-reference ICU/CLDR/Unicode combination receives a distinct numeric algorithm identity and is separated in evidence. Japanese kana and Korean Hangul are not currently tokenized. Candidate-corroborated capability-gap search hints remain a separate display path: generic and broad concepts do not satisfy its evidence gate, and low-confidence unsegmented two-character Han fragments remain empty. Completion Hooks record content-free turn/session reasons only in opt-in learning mode. Invalid input, missing state, timeout, HMAC failure, or evidence-write failure returns protocol-valid non-blocking JSON so the Harness continues normally.

`skill-steward preflight --stdin --compact-json` is the Harness/Skill handoff contract. Compact schema v4 emits one JSON line within 4,096 UTF-8 bytes and carries selected use/install recommendations, stable reason/warning codes, coverage, context, and a nullable feedback command. The command is `null` only when evidence persistence failed, paired with `PREFLIGHT_PERSISTENCE_UNAVAILABLE`. Raw task text, capability lists, full candidate features, and readable reasons stay out of this compact payload. Full result schema v5 adds numeric capability coverage/precision and trigger confidence to the complete `PreflightResult`; available catalog candidates may retain catalog `source` metadata. It does not embed native inventory source, ownership, plugin, or exposure records. Portfolio reports and the dashboard preserve those records, while Preflight represents resolved visibility with candidate reason codes and inventory warnings. Both paths use the same privacy-reduced evidence boundary and never persist raw task text.
...
Evidence defaults to `minimal`. A fingerprint-bound, expiring plan is required before enabling `learning`, which adds numeric candidate features—including capability coverage/precision and trigger confidence—and HMAC-pseudonymous lifecycle events. Capability names and action-object pairs are not valid evidence fields. Raw prompts, terms, paths, Harness IDs, transcripts, assistant content, and tool data are also excluded. The 32-byte salt is private, is never exported, and is removed only by an exact evidence-erase plan.
...
The recovery reader classifies that persisted state once for every surface. An exact current lifecycle-record binding selects the full idempotent finalize path; exact stable non-publication selects rollback; unstable, contradictory, or unreadable evidence selects `unknown` and exposes no mutation. CLI, loopback API, and Dashboard can review and apply the resulting single-use plan, but cannot choose the opposite direction or force a write. Apply claims the plan under the shared lease, revalidates its transaction sequence and evidence digest, and reopens companion artifacts only through Store-issued one-shot authority. A partial retry returns a sanitized `recovery-required` receipt and requires a fresh plan.
...
POSIX journal publication fsyncs the verified records directory before returning its opaque commit receipt. Windows does not provide the same directory-handle fsync through Node, so the compatibility journal revalidates directory device, inode, physical containment, fragment identity, and complete journal state without calling the unsupported operation. Phase 7 runs those guarantees against native Windows identities and junctions in CI. It still does not activate Windows integration mutation: plans remain unavailable until production has both native reparse detection and handle-relative no-replace move/removal authority.
