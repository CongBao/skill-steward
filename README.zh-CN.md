...
算法 v8 是在本机运行的确定性词法排序，不依赖 LLM，也不是通用语义搜索。它会综合相关性、独特覆盖、风险、冗余、Harness 兼容性和上下文估算；条件接近时优先使用已经安装的 Skill。有限的简繁中文概念和严格的合并前代码评审信号用于改善常见路由；“不要评审”一类明确否定会排除相关工作流。能力缺口只有在证据足够具体时才显示，因此低置信输入可以不提供建议。精确的解析、触发和对抗性边界见[架构说明](docs/architecture.md#task-time-data-flow)与[测试协议](docs/alpha-testing.md#compact-and-bilingual-preflight)。
...
Skill Steward 可以接入 Codex、Claude Code 和 GitHub Copilot CLI，而不取代它们。托管 Hook 负责观察原生生命周期，共享配套 Skill 则让 Harness 能显式调用任务预检。JSON 状态会把这两部分分开显示，因此 Hook 已连接时，缺失、过期、被修改或无法读取的配套 Skill 不会被掩盖：
