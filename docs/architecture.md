...
Integration apply and remove acquire `integration-mutation.lease` before claiming a reviewed plan or changing managed files. The state-scoped lease serializes CLI and dashboard mutations across processes, remains owned through configuration, journal commit, initial scan, compensation, and shared companion cleanup, and reports busy without consuming the waiting plan. A caller revalidates its plan after entering the lease, so a stale concurrent plan fails on drift rather than racing a completed transaction.
