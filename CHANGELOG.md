...
- Harness integration setup now exposes proof-aware plans and separate Hook/companion status while public apply remains disabled. CLI and Dashboard plans report that boundary explicitly; apply consumes and revalidates a reviewed plan, then refuses before any Hook or companion write. The legacy cleanup path removes only a provably managed Hook and retains the shared companion.
