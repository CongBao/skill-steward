...
- 仓库已经包含一条按受保护评审设计的预发布工作流：它直接复用 npm 实际提供的七个 tarball，核对 SHA-256/SHA-512 和规范化清单，并在 GitHub 草稿转为公开前为完整九文件资产生成证明。这条流程尚未实际产出公开 npm 包或 GitHub Release，因此目前不宣称受保护环境和公开安装已经验证。
...
4. 完成第一次受保护的 npm 与带证明的 GitHub Beta 发布，并在更新发布状态前验证公开安装流程。

版本变化见 [CHANGELOG.md](CHANGELOG.md)，发布控制见 [GitHub 预发布说明](docs/github-prerelease.md)；历史 Alpha.3 结论、真实操作证据、基线和优先级见 [2026-07-03 产品评审](docs/product-review-2026-07-03.md)。
